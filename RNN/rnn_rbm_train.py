...
saved_weights_path = (sys.path[
                          0] + '/parameter_checkpoints/initialized.ckpt')  # The path to the initialized weights checkpoint file
...
def main(num_epochs, songList):
    ...
    gvs = [(tf.clip_by_value(grad, -10., 10.), var) for grad, var in
           gvs]  # We use gradient clipping to prevent gradients from blowing up during training
    updt = opt_func.apply_gradients(
        gvs)  # The update step involves applying the clipped gradients to the model parameters

    songList = songList.split(',')
    songs = midi_manipulation.get_songs(songList)  # Load the songs

    saver = tf.train.Saver(
        tvars)  # We use this saver object to restore the weights of the model and save the weights every few epochs

    with tf.Session() as sess:
        init = tf.global_variables_initializer()
        sess.run(init)
        saver.restore(sess,
                      saved_weights_path)  # Here we load the initial weights of the model that we created with weight_initializations.py
        ...
        for epoch in range(num_epochs):
            ...
            for s_ind, song in enumerate(songs):
                for i in range(1, len(song), batch_size):
                    ...
                    alpha = min(.05, 0.1 / float(i))  # We decrease the learning rate according to a schedule.
